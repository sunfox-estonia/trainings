...
$f3->route('GET /plan/@pql', function ($f3, $params) {
    ...
    if ($pln->comments) {
        $r = 0;
        $plan_comments_prepared = null;
        foreach (json_decode($pln->comments) as $txt) {
            $plan_comments_prepared[$r] = $txt;
            $r++;
        }
    }
    $f3->set('pln_comments', $plan_comments_prepared);
...
/**
 * Update plan settings in a PLANS table.STES
 */
...
$f3->route('POST /update/plan', function ($f3) {
    ...
    $plan_comments = $f3->get('POST.comments_data');
    $pln = new DB\SQL\Mapper($f3->get('DB'), 'plans');
    $pln->load(array('id=?', $plan_id));
    $pln->content = $plan_data;
    $pln->comments = $plan_comments;
    $pln->save();
    echo "success";
});
